...
Hàm tính trước tensor codebook
$\mathbf{C} \in \mathbb{C}^{P \times v \times N_{i_2} \times N_{i_{1,1}} \times N_{i_{1,2}} \times N_{i_{1,3}}}$
và trả về cho \texttt{nrPMIReport} để tìm PMI bằng exhaustive SINR search.
Tổng số ứng viên hạng~1: $4 \times 64 \times 16 \times 4 = 4{,}096$.

\subsection{Hàm Mode B: Greedy Factored Search}

Kích thước không gian tìm kiếm Mode~B tăng theo lũy thừa với hạng:
hạng~2 yêu cầu $\sim$4,3\,GB tensor --- không khả thi để tính trước.
Thuật toán Greedy Factored Search giải quyết bằng ba giai đoạn:

\begin{enumerate}
\item Liệt kê $O_1 O_2 = 16$ cặp offset phân số $(q_1, q_2)$.
\item Với mỗi $(q_1, q_2)$:
  \begin{itemize}
    \item Hạng~1: duyệt vét cạn $N_1 N_2 = 64$ beam (đảm bảo tối ưu = Mode~A).
    \item Hạng~$v > 1$: dùng SVD wideband $\mathbf{H}_{wb}$ để chọn 1 beam tốt
          nhất cho mỗi tầng. Điểm số tính trên \emph{cả hai phân cực}:
          \begin{equation}
            \text{score}(n,l) = |\mathbf{V}_q^H\mathbf{V}_\text{top}|_{n,l}^2
                               + |\mathbf{V}_q^H\mathbf{V}_\text{bot}|_{n,l}^2
          \end{equation}
          Chỉ dùng phân cực trên sẽ mất $\approx50\%$ thông tin trên kênh Rayleigh.
  \end{itemize}
\item Duyệt vét cạn $4^v$ tổ hợp co-phase; chọn $W$ tối đa hoá dung lượng
      $\log_2\det(\mathbf{I} + \frac{1}{\sigma^2}\mathbf{H}_{wb}\mathbf{W}\mathbf{W}^H\mathbf{H}_{wb}^H)$.
\end{enumerate}

Độ phức tạp: hạng~4 chỉ cần $16\times(64+256)=5{,}120$ đánh giá thay vì
$\sim 6{,}9\times10^{10}$ (vét cạn đầy đủ).

\subsection{Kiểm tra xác nhận Mode B (5 test PASS)}

Script \texttt{test\_ModeB\_verification.m} kiểm tra 5 tiêu chí:
ánh xạ chỉ số, chuẩn hoá công suất $\|W\|_F^2=1$, cấu trúc co-phase,
phục hồi beam kênh LoS, và $\text{cap}(\text{Mode B}) \geq \text{cap}(\text{Mode A})$.
Kết quả quan trọng nhất:

\begin{table}[h!]
\centering
\caption{TEST 5: cap(Mode B) $\geq$ cap(Mode A) --- 20 kênh Rayleigh, hạng~1}
\label{tab:test5}
\begin{tabular}{lcc}
\toprule
Phiên bản thuật toán & Mode B $>$ Mode A & Mode B $=$ Mode A \\
\midrule
Ban đầu (greedy, chỉ $V_\text{top}$) & 0\,\% & 15\,\% \\
Sửa scoring ($V_\text{top}+V_\text{bot}$) & 0\,\% & 15\,\% \\
Sửa hạng~1 (vét cạn 64 beam/offset) & 0\,\% & \textbf{100\,\%} \\
\bottomrule
\end{tabular}
\end{table}
...
\subsection{Vì sao \texttt{nrDLPMISelect} không dùng được cho Rel-19}

\texttt{nrRISelect} gọi một trong hai hàm nội bộ:
\begin{itemize}[noitemsep]
  \item \texttt{riSelectPMI}: dùng \texttt{nrDLPMISelect} với tiêu chí MaxSINR.
  \item \texttt{riSelectCQI}: dùng \texttt{nrDLPMISelect} với tiêu chí MaxSE.
\end{itemize}
Cả hai đều gọi \texttt{nrDLPMISelect}, vốn không chấp nhận
\texttt{CodebookType = 'typeI-SinglePanel-r19'}. Giải pháp là thêm một nhánh
thứ ba \texttt{riSelectR19} dùng \texttt{nrPMIReport} thay thế.
